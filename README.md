...
- Multiple single-cell downstream analyses:
  ...
  - Spatial analysis: modular domain, feature, neighborhood, integration, deconvolution, and SpatialCellChat communication workflows; native spatial networks and cell-boundary visualization; and lightweight bridges to Giotto and SPATA2 without wrapping their full frameworks. Use `ListSpatialMethods()`, `SpatialBackendStatus()`, and `SpatialResultInfo()` to discover methods, diagnose optional backends, and inspect stored results; use `SpatialCoordinates()`, `GetSpatialResult()`, and `GetSpatialGraph()` for stable read-only access to coordinates, result bundles, and graphs. See the [Giotto compatibility and framework bridge guide](vignettes/spatial-framework-bridges.Rmd) before choosing between the legacy workflow and native bridges.
