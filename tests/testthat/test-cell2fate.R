...
test_that("Cell2fate runner treats a malformed resume manifest as a cache miss", {
  ...
  writeLines(
    c(
      ...
      "assert module['RUNNER_DIR'] not in [Path(value or os.curdir).resolve() for value in sys.path]",
      "fake_backend = types.ModuleType('cell2fate')",
      "fake_backend.__file__ = str(Path(tempfile.gettempdir()) / 'cell2fate' / '__init__.py')",
      "fake_backend.__path__ = []",
      "fake_utils = types.ModuleType('cell2fate.utils')",
      "sys.modules['cell2fate'] = fake_backend",
      "sys.modules['cell2fate.utils'] = fake_utils",
      "loaded_backend, loaded_utils = module['_load_cell2fate_backend']()",
      "assert loaded_backend is fake_backend",
      "assert loaded_utils is fake_utils",
      "del sys.modules['cell2fate.utils']",
      "del sys.modules['cell2fate']",
