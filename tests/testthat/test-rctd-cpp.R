...
test_that("spacexr backend exposes legacy RCTD symbols", {
  spec <- scop:::spatial_backend_registry()[["spacexr"]]
  expected <- c("SpatialRNA", "Reference", "create.RCTD", "run.RCTD")
  expect_identical(spec$symbols, expected)
  expect_identical(
    scop:::spatial_backend_required_symbols(spec, exports = expected),
    expected
  )
  expect_identical(
    scop:::spatial_backend_required_symbols(spec, exports = c("create.RCTD", "run.RCTD")),
    expected
  )
})
