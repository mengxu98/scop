...
test_that("CytoTRACE2 reloads replaced and removed model assets at the same path", {
  data_dir <- tempfile()
  dir.create(data_dir)
  on.exit(unlink(data_dir, recursive = TRUE), add = TRUE)
  saveRDS(list(version = 1L), file.path(data_dir, "model_parameters.rds"))
  features <- file.path(data_dir, "features_model_training_17.csv")
  writeLines(c(",0", "0,A1bg"), features)
  first <- load_cytotrace2_data(data_dir, verbose = FALSE)
  expect_identical(load_cytotrace2_data(data_dir, verbose = FALSE), first)
  stamp <- file.info(features)$mtime
  writeLines(c(",0", "0,A1cf"), features)
  Sys.setFileTime(features, stamp)
  expect_identical(load_cytotrace2_data(data_dir, verbose = FALSE)$features, "A1cf")
  mapping <- file.path(data_dir, "mt_human_alias.csv")
  writeLines(c("alias,gene", "old,A1cf"), mapping)
  expect_false(is.null(load_cytotrace2_data(data_dir, verbose = FALSE)$alias_dict))
  unlink(mapping)
  expect_null(load_cytotrace2_data(data_dir, verbose = FALSE)$alias_dict)
  saveRDS(list(version = 2L), file.path(data_dir, "model_parameters.rds"))
  expect_identical(load_cytotrace2_data(data_dir, verbose = FALSE)$parameter_dict$version, 2L)
  unlink(features)
  expect_error(load_cytotrace2_data(data_dir, verbose = FALSE), "Feature file not found")
})
