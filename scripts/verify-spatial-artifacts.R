...
for (i in seq_len(nrow(tab))) {
  ...
  plot <- switch(name,
    ...
    hd = SpatialSpotPlot(object, features = rownames(object)[1], assay = "Spatial.008um",
      layer = "counts", image = "slice1.008um"),
    xenium = SpatialCellPlot(object, image = "fov"))
