...
run_scomm_subprocess <- function(
  ...
  utils::write.table(train_x, train_x_file, sep = ",", row.names = FALSE, col.names = FALSE)
  utils::write.table(as.matrix(split_data$train_y), train_y_file, sep = ",", row.names = FALSE, col.names = FALSE)
  utils::write.table(query_x, query_x_file, sep = ",", row.names = FALSE, col.names = FALSE)
  ...
  probs <- as.matrix(utils::read.csv(prob_file, header = FALSE, check.names = FALSE))
  ...
  list(
    ...
    probabilities = as.data.frame(probs, check.names = FALSE, stringsAsFactors = FALSE)
  )
}
