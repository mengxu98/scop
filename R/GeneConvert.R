  ...
  if (species_from != species_to) {
    for (from_attr in from_IDtype) {
      if (length(geneID) > 0) {
        ...
        if (all(geneID_to_IDtype %in% c("symbol", "ensembl_id"))) {
          ...
          geneID_res2$to_IDtype <- setNames(names(to_attr), nm = to_attr)[
          ...
          geneID_res3$to_IDtype <- setNames(names(to_attr), nm = to_attr)[
    ...
    for (from_attr in from_IDtype) {
      if (length(geneID) > 0) {
        ...
        geneID_res1_to$to_IDtype <- setNames(names(to_attr), nm = to_attr)[
