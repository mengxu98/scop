...
check_srt_list <- function(
  ...
  assay_type <- unique(sapply(srt_list, function(srt) class(srt[[assay]])))
  ...
  features_list <- lapply(srt_list, function(srt) {
  ...
  if (length(unique(features_list)) != 1) {
    ...
    cf <- Reduce(
      intersect,
      lapply(srt_list, function(srt) rownames(srt[[assay]]))
    )
    warning(
  ...
  for (i in seq_along(srt_list)) {
    if (!assay %in% Seurat::Assays(srt_list[[i]])) {
    ...
    if (normalization_method %in% c("SCT") && type == "RNA") {
      ...
      if (
        !"residual_variance" %in%
          colnames(srt_list[[i]]@assays$SCT@meta.features)
      ) {
        ...
        feature.attr <- srt_list[[i]]@assays$SCT@meta.features
      }
