...
#'
#' @description
#' Generates various types of plots for Gene Set Enrichment Analysis (GSEA) results.
#'
#' @md
...
#' @param object A `Seurat` object containing the results of RunDEtest and RunGSEA.
...
#' If provided, 'object', 'test.use' and 'group.by' are ignored.
#' @param group.by Grouping variable in the `Seurat` object.
#' This argument is only used if `object` is specified.
#' `NULL` (the default) selects the results that [RunGSEA] stored for custom
#' gene sets or custom cell groups, which are keyed as `"custom"`.
#' @param plot_type The type of plot to generate.
#' Options are: "line", "comparison", "bar", "network", "enrichmap", "wordcloud".
#' @param direction The direction of enrichment to include in the plot.
#' Must be one of "pos", "neg", or "both".
...
#' @param features_label Feature names to include as labels in the line plot.
...
GSEAPlot <- function(
  ...
  if (is.null(res)) {
    if (is.null(srt)) {
      log_message(
        "Either {.arg object} or {.arg res} must be provided",
        message_type = "error"
      )
    }
    layer <- paste("GSEA", group.by %||% "custom", test.use, sep = "_")
    if (!layer %in% names(srt@tools)) {
      available <- grep("^GSEA", names(srt@tools), value = TRUE)
      ...
      log_message(
        "No enrichment result {.val {layer}} found in {.code srt@tools}. ",
        "Available results: {.val {available}}. Set {.arg group.by} and {.arg test.use} ",
        "to match the {.fn RunGSEA} call; custom gene sets and custom cell groups ",
        "are stored under {.arg group.by} {.val custom}.",
        message_type = "error"
      )
    }
