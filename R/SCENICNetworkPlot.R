...
scenic_plot_egrn <- function(
  ...
  triplets <- scenic_get_triplets(srt, tool_name)
  if (is.null(triplets)) {
    log_message(
...
scenic_get_triplets <- function(srt, tool_name) {
...
scenic_network_edges <- function(
  ...
  triplets <- if (isTRUE(include_regions)) {
    scenic_get_triplets(srt, tool_name)
  } else {
    NULL
  }
  ...
  adjacency <- scenic_top_edges(adjacency, max_targets = max_targets)
  adjacency <- scenic_limit_edges(adjacency, max_edges = max_edges)
  scenic_adjacency_edges(adjacency)
}

scenic_adjacency_edges <- function(adjacency) {
  weight <- as.numeric(adjacency[["importance"]])
  weight[!is.finite(weight)] <- 1
  scenic_clean_edges(data.frame(
    from = as.character(adjacency[["TF"]]),
    to = as.character(adjacency[["target"]]),
    weight = weight,
    edge_type = "tf_gene",
...
scenic_triplet_edges <- function(triplets, max_targets = 20) {
  if ("score" %in% colnames(triplets)) {
    triplets <- triplets[order(abs(triplets[["score"]]), decreasing = TRUE), , drop = FALSE]
  }
  kept <- do.call(
    rbind,
    lapply(split(triplets, triplets[["TF"]]), function(df) {
      genes <- utils::head(unique(as.character(df[["gene"]])), max_targets)
  ...
  weight <- if ("score" %in% colnames(kept)) as.numeric(kept[["score"]]) else rep(1, nrow(kept))
  weight[!is.finite(weight)] <- 1
  edge_sign <- ifelse(weight < 0, "negative", "positive")
  scenic_aggregate_edges(rbind(
    data.frame(
...
scenic_network_layout <- function(graph, layout = "kk", node_type = NULL) {
  ...
  xy <- switch(
    ...
    scenic_layout_force(graph, method = "kk")
  )
  if (is.null(dim(xy))) {
    xy <- matrix(xy, ncol = 2)
  }
...
scenic_layout_hubs <- function(graph, node_type) {
  ...
  n_tf <- length(tfs)
  if (n_tf == 0L) {
    xy <- igraph::layout_in_circle(graph)
    if (is.null(rownames(xy))) {
      rownames(xy) <- names
    }
    return(xy)
...
scenic_layout_tripartite <- function(graph, node_type) {
  ...
  parents <- scenic_network_parent_tfs(graph, node_type)
  tfs <- sort(unique(as.character(tfs)))
...
scenic_layout_bipartite <- function(graph, node_type) {
  names <- igraph::V(graph)$name
  tfs <- sort(unique(as.character(names[node_type[names] == "TF"])))
