...
#' @param signalMode `receiving`, `sending`, or `both` for spot-level ST.
...
secact_store_activity_assay <- function(
  ...
  mat <- as.matrix(activity_res[[activity]])
  if (is.null(rownames(mat)) || is.null(colnames(mat))) {
    log_message(
      "{.pkg SecAct} activity matrix must have row and column names",
      message_type = "error"
    )
  }
  cells <- colnames(srt)
