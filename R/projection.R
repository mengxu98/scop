...
RunKNNMap <- function(
  ...
  if (is.null(ref_umap)) {
    ref_umap <- sort(Reductions(srt_ref)[grep(
      "umap",
      Reductions(srt_ref),
      ignore.case = TRUE
    )])[1]
  ...
  if (!is.null(query_reduction) && !is.null(ref_reduction)) {
    ...
    if (length(features) == 0) {
      ...
      features <- intersect(
        VariableFeatures(srt_query, assay = query_assay),
        VariableFeatures(srt_ref, assay = ref_assay)
      )
    }
    ...
    query <- t(GetAssayData(srt_query, layer = "data", assay = query_assay)[
      features_common,
    ])
    ref <- t(GetAssayData(srt_ref, layer = "data", assay = ref_assay)[
      features_common,
    ])
  }
  ...
  if (nn_method %in% c("annoy", "rann")) {
    ...
    if (k == 1) {
      ...
      match_k <- t(as.matrix(apply(
        d,
        2,
        function(x) order(x, decreasing = FALSE)[1:k]
      )))
      match_k_cell <- t(as.matrix(apply(
        d,
        2,
        function(x) names(x)[order(x, decreasing = FALSE)[1:k]]
      )))
      match_k_distance <- t(as.matrix(apply(
        d,
        2,
  ...
  if (projection_method == "model") {
    ...
    srt_query[["ref.embeddings"]] <- CreateDimReducObject(
      embeddings = refumap,
      key = "Dim_",
      assay = query_assay,
  ...
  if (!is.null(ref_group)) {
    ...
    if (k == 1) {
      ...
      if (!inherits(match_freq, "list")) {
        ...
        match_freq <- lapply(
          setNames(names(match_freq), names(match_freq)),
          function(x) setNames(k, match_k_cell[x, 1])
        )
      }
...
RunPCAMap <- function(
  ...
  if (is.null(ref_pca)) {
    ref_pca <- sort(Reductions(srt_ref)[grep(
      "pca",
      Reductions(srt_ref),
      ignore.case = TRUE
    )])[1]
  ...
  center <- apply(
    GetAssayData(object = srt_ref, layer = "data", assay = ref_assay)[
      features,
    ],
    1,
    mean
  ...
  query_data <- t(GetAssayData(srt_query, layer = "data", assay = query_assay)[
    features_common,
  ])
  query_pca <- scale(
...
RunCSSMap <- function(
  ...
  if (is.null(ref_css)) {
    ref_css <- sort(Reductions(srt_ref)[grep(
      "css",
      Reductions(srt_ref),
      ignore.case = TRUE
    )])[1]
...
RunSymphonyMap <- function(
  ...
  if (is.null(ref_harmony)) {
    ref_harmony <- sort(Reductions(srt_ref)[grep(
      "harmony",
      Reductions(srt_ref),
      ignore.case = TRUE
    )])[1]
  ...
  res <- mapQuery(
    exp_query = GetAssayData(srt_query, layer = "data", assay = query_assay),
...
buildReferenceFromSeurat <- function(
  ...
  if (assay == "RNA") {
    vargenes_means_sds <- data.frame(
      ...
      mean = rowMeans(obj@assays[[assay]]@data[
        obj@assays[[assay]]@var.features,
      ])
    )
    ...
    vargenes_means_sds <- data.frame(
      symbol = obj@assays[[assay]]@var.features,
      mean = rowMeans(obj@assays[[assay]]@scale.data[
        obj@assays[[assay]]@var.features,
      ])
    )
    asdgc <- Matrix(
      obj@assays[[assay]]@scale.data[obj@assays[[assay]]@var.features, ],
      sparse = TRUE
    )
    vargenes_means_sds$stddev <- symphony::rowSDs(
      asdgc,
  ...
  res$centroids <- t(symphony:::cosine_normalize_cpp(
    V = res$R %*% t(res$Z_corr),
    dim = 1
  ))
