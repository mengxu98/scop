...
#' Default is FALSE.
#' @param convert_species A logical value indicating whether to use a species-converted database if annotation is missing for \code{species}.
#' Default is FALSE.
...
#' The size of dot represents percentage of expressed cells based on the specified \code{exp_cutoff}.
#' Default is FALSE.
...
GroupHeatmap <- function(
  ...
  if (!is.null(cell_annotation)) {
    ...
    if (
      any(
        !cell_annotation %in%
          c(colnames(srt@meta.data), rownames(
            Seurat::GetAssay(
              srt,
              assay = assay
            )
  ...
  feature_metadata <- cbind.data.frame(
    ...
    GetFeaturesData(srt, assay = assay)[
      ...
      intersect(
        feature_annotation,
        colnames(GetFeaturesData(srt, assay = assay))
      ),
      drop = FALSE
    ]
  )
  ...
  features_ordered <- rownames(mat_list[[1]])[unlist(
    suppressWarnings(
    ...
    for (i in seq_along(feature_annotation)) {
      ...
      if (!is.numeric(featan_values)) {
        ...
        col_fun <- circlize::colorRamp2(
          ...
          colors = palette_scop(palette = palette[i], palcolor = palcolor)
        )
        ha_feature <- list()
        ha_feature[[featan]] <- ComplexHeatmap::anno_block(
          align_to = split(seq_along(featan_values), featan_values),
  ...
  for (cell_group in group.by) {
    ...
    funbody <- paste0(
      ...
      if (isTRUE(add_reticle)) {
        ...
        for(col in seq_len(ncol(ind_mat))){
          grid::grid.lines(x = grid::unit(rep(x[ind_top[col]],each=2),'npc'),y = grid::unit(c(0,1),'npc'),gp = grid::gpar(col = '",
          reticle_color,
          "', lwd = 1.5));
        };
        for(row in seq_len(nrow(ind_mat))){
          grid::grid.lines(x = grid::unit(c(0,1),'npc'),y = grid::unit(rep(y[ind_left[row]],each=2),'npc'),gp = grid::gpar(col = '",
          reticle_color,
          "', lwd = 1.5));
        };
        "
    ...
    ht_args <- list(
      ...
      col = colors,
      layer_fun = methods::getFunction("layer_fun", where = environment()),
      row_title = row_title %||%
        if (flip) {
