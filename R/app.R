...
FetchH5 <- function(
  ...
  if (!is.null(reduction)) {
    ...
    for (i in reduction) {
      ...
      colnames(reduction) <- paste0(
        as.character(reduction_attr$key),
        seq_len(ncol(reduction))
      )
      srt_tmp@reductions[[i]] <- SeuratObject::CreateDimReducObject(
...
theme_list <- list(
  scop = c("theme_scop", "theme_spatial", "theme_blank"),
  ggplot2 = c(
