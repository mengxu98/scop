ExpressionStatPlot <- function(
  ...
  if (plot_type == "col") {
    if (
      ...
      log_message(
        "Cannot add other layers when {.arg plot_type} is set to {.val col}",
  ...
  if (
    ...
    log_message(
      "Cannot add comparison when {.arg plot_type} is set to {.val col}",
      message_type = "warning"
    )
    multiplegroup_comparisons <- FALSE
    comparisons <- NULL
  ...
  if (isTRUE(comparisons) && is.null(split.by)) {
    log_message(
      "{.arg split.by} must provided when {.arg comparisons} is set to {.pkg TRUE}",
      message_type = "error"
    )
  }
  ...
  if (!is.null(cells.highlight) && isFALSE(cells.highlight)) {
    if (!any(cells.highlight %in% allcells)) {
      log_message(
        "No cells in {.arg cells.highlight} found",
        message_type = "error"
      )
    }
    if (!all(cells.highlight %in% allcells)) {
      log_message(
        "Some cells in {.arg cells.highlight} not found",
        message_type = "warning"
      )
    }
  ...
  if (!is.null(cells.highlight) && isFALSE(add_point)) {
    log_message(
      "{.arg cells.highlight} is valid only when {.arg add_point} is set to {.pkg TRUE}",
      message_type = "warning"
    )
  }
  if (isTRUE(stack) && isTRUE(sort)) {
    log_message(
      "Set {.arg sort} to {.pkg FALSE} when {.arg stack} is set to {.pkg TRUE}",
      message_type = "warning"
    )
    sort <- FALSE
  }
  if (isTRUE(multiplegroup_comparisons) || length(comparisons) > 0) {
    ...
    if (any(ncomp > 2)) {
      log_message(
        "{.arg comparisons} must be a list in which all elements must be vectors of length 2",
        message_type = "error"
      )
    }
  }
  ...
  if (length(features_drop) > 0) {
    log_message(
      "{.val {features_drop}} are not found",
      message_type = "warning"
    )
  ...
  if (isTRUE(calculate_coexp) && length(features_gene) > 1) {
    if (length(features_meta) > 0) {
      log_message(
        "{.val {features_meta}} are not used when calculating co-expression",
        message_type = "warning"
      )
    }
  ...
  if (!is.numeric(dat_exp) && !inherits(dat_exp, "Matrix")) {
    log_message(
      "{.arg stat.by} must be type of numeric variable",
      message_type = "error"
    )
  }
  dat_group <- meta.data[,
  ...
  plist <- lapply(
    stats::setNames(rownames(comb), rownames(comb)), function(i) {
      ...
      dat[["group.unique"]] <- utils::head(
        factor(
          paste(
            "sp", dat[["split.by"]], "gp", dat[["group.by"]],
            sep = "-"
          ),
          levels = paste(
            "sp", group_comb[[1]], "gp", group_comb[[2]],
            sep = "-"
          )
        ),
        nrow(dat)
      ...
      if (length(comparisons) > 0) {
        if (isTRUE(comparisons)) {
          ...
          if (any(row_sums >= 3)) {
            log_message(
              "Detected more than 2 groups. Use {.val {multiple_method}} for comparison"
            )
