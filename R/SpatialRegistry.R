...
spatial_method_registry <- function() {
  ...
  rows <- list(
    ...
    entry(
      ...
      "bridge",
      "framework_bridge",
      "SpatialFrameworkConvert.R",
      backend_id = "giotto;giotto_class",
      coordinate_space_current = "raw",
      coordinate_requirement = "backend_managed"
    ),
    entry(
    ...
    entry(
      ...
      "deconvolution",
      "RunDeconvolution.R",
      backend_id = "music;bisquerna;bayesprism;cibersort",
      coordinate_space_current = "none",
      coordinate_space_target = "none",
      coordinate_requirement = "identity_only",
      plot_function = "DeconvolutionPlot",
      backend_requirement = "any"
    ),
    entry(
      "RunRCTD",
      "analysis",
    ...
    entry(
      ...
      coordinate_space_target = "none",
      coordinate_requirement = "identity_only",
      plot_function = "STdeconvolvePlot"
    ),
    entry(
      "RunSPOTlight",
      "analysis",
    ...
    entry(
      ...
      coordinate_space_target = "none",
      coordinate_requirement = "identity_only",
      plot_function = "Cell2locationPlot"
    ),
    entry(
      "RunSpatialDWLS",
      "analysis",
    ...
    entry(
      ...
      "analysis",
      "ecotype",
      "RunSpatialEcoTyper.R",
      "SpatialEcoTyper",
      "spatialecotyper",
      coordinate_space_current = "none",
      coordinate_space_target = "none",
      coordinate_requirement = "identity_only",
      plot_function = "SpatialEcoTyperSpatialPlot"
    ),
    ...
    entry(
      ...
      coordinate_space_target = "raw",
      coordinate_requirement = "distance_sensitive",
      plot_function = "SpatialVariableFeaturePlot"
    ),
    entry(
      "RunSpatialVariableFeatures",
      "analysis",
      "feature_pattern",
      "RunSpatialVariableFeatures.R",
    ...
    entry(
      ...
      "analysis",
      "feature_pattern",
      "RunSpatialGradientFeatures.R",
      "SpatialGradientFeatures",
      "core;spata2",
      coordinate_space_current = "legacy_display",
      coordinate_space_target = "raw",
      coordinate_requirement = "distance_sensitive",
      plot_function = "SpatialGradientPlot",
      backend_requirement = "any"
    ),
    ...
    entry(
      ...
      coordinate_requirement = "distance_sensitive",
      scalability = "sparse",
      plot_function = "SpatialNetworkPlot"
    ),
    entry(
      "RunSpatialNeighborhood",
      "analysis",
      "neighborhood",
      "RunSpatialNeighborhood.R",
      "SpatialNeighborhood",
      "core;spicyr",
      coordinate_space_current = "legacy_display",
      coordinate_space_target = "raw",
      coordinate_requirement = "distance_sensitive",
      scalability = "sparse_required",
      plot_function = "SpatialNeighborhoodPlot",
      backend_requirement = "any"
    ),
    entry(
      "RunSpatialCellChat",
      "analysis",
      "communication",
      "RunSpatialCellChat.R",
      "SpatialCellChat",
      "spatialcellchat",
      coordinate_space_current = "raw",
      coordinate_requirement = "distance_sensitive",
      scalability = "sparse_required",
      plot_function = "SpatialCellChatPlot"
    ),
    entry(
      "RunStatialKontextual",
      "analysis",
      "neighborhood",
      "RunStatialKontextual.R",
      "StatialKontextual",
      "statial",
      coordinate_space_current = "legacy_display",
      coordinate_space_target = "raw",
      coordinate_requirement = "distance_sensitive",
      plot_function = "StatialKontextualPlot"
    ),
    entry(
      "RunSpatialIntegration",
      "analysis",
      "integration",
      "RunSpatialIntegration.R",
      "SpatialIntegration",
      "precast;bass;spatialmnn",
      coordinate_space_current = "legacy_display",
      coordinate_space_target = "raw",
      coordinate_requirement = "distance_sensitive",
      plot_function = "SpatialIntegrationPlot",
      backend_requirement = "any"
    ),
    entry(
      "RunMistyR",
...
spatial_backend_registry <- function() {
  ...
  list(
    ...
    biocneighbors = backend(
      "biocneighbors",
      "BiocNeighbors",
      symbols = c("findKNN", "findNeighbors")
    ),
    ...
    giotto = backend(
      ...
      "Giotto",
      "drieslab/Giotto",
      giotto_symbols$symbol[
        giotto_symbols$package == "Giotto" & giotto_symbols$required
      ]
    ),
    giotto_class = backend(
      ...
      giotto_symbols$symbol[
        giotto_symbols$package == "GiottoClass" & giotto_symbols$required
      ]
    ),
    spata2 = backend(
      "spata2",
    ...
    bisquerna = backend(
      "bisquerna",
      "BisqueRNA",
      symbols = "ReferenceBasedDecomposition"
    ),
    bayesprism = backend(
      "bayesprism",
      "BayesPrism",
      symbols = c("new.prism", "run.prism")
    ),
    cibersort = backend(
      "cibersort",
      "CIBERSORT",
...
ListSpatialMethods <- function(
  ...
  if (!is.null(pattern)) {
    if (
      ...
      log_message(
        "{.arg pattern} must be one non-empty string",
        message_type = "error"
      )
    }
    haystack <- apply(
      registry[,
        c("method", "task", "backend_id", "documentation"),
        drop = FALSE
      ],
      1,
      paste,
  ...
  if (!is.null(available)) {
    if (!is.logical(available) || length(available) != 1L || is.na(available)) {
      log_message(
        "{.arg available} must be TRUE, FALSE, or NULL",
        message_type = "error"
      )
    }
    status_table <- SpatialBackendStatus(api_check = TRUE)
    ready <- stats::setNames(
      status_table$availability == "available",
      status_table$backend_id
    )
    method_ready <- vapply(
...
spatial_backend_cache_key <- function(
  api_check,
  backend_ids,
  method,
  envname,
...
spatial_backend_readonly_conda <- function(conda = "auto") {
  if (
    ...
    log_message(
      "{.arg conda} must be one non-empty string",
      message_type = "error"
    )
  }
...
spatial_python_backend_status <- function(spec, envname, conda, api_check) {
  ...
  missing <- if (isTRUE(api_check)) {
    names(package_status)[!package_status]
  } else {
    character()
  }
  installed <- environment_exists &&
    (!isTRUE(api_check) || length(missing) == 0L)
...
spatial_backend_required_symbols <- function(
  spec,
  method = NULL,
  exports = NULL
) {
...
SpatialBackendStatus <- function(
  ...
  if (!is.logical(api_check) || length(api_check) != 1L || is.na(api_check)) {
    log_message(
      "{.arg api_check} must be TRUE or FALSE",
      message_type = "error"
    )
  }
  ...
  if (length(backends) == 0L) {
    return(data.frame(
      ...
      installed = logical(),
      api_checked = logical(),
      required_symbols = character(),
      missing_symbols = character(),
      availability = character(),
      namespace_error = character(),
      stringsAsFactors = FALSE
    ))
  }
  ...
  rows <- lapply(backends, function(spec) {
    ...
    if (!is_installed) {
      ...
      if (is.null(exports)) {
        ...
        availability <- if (length(missing_symbols) == 0L) {
          "available"
        } else {
          "api_incompatible"
        }
...
spatial_result_build <- function(
  ...
  if (
    ...
    log_message(
      "{.arg method} must be one non-empty result family",
      message_type = "error"
    )
  }
  if (
    !is.character(result_type) ||
      length(result_type) != 1L ||
      is.na(result_type) ||
      !nzchar(result_type)
  ) {
    log_message(
      "{.arg result_type} must be one non-empty registry task",
      message_type = "error"
    )
  }
  if (!is.list(source) || !is.list(provenance)) {
    log_message(
      "{.arg source} and {.arg provenance} must be lists",
      message_type = "error"
    )
  }
  source_input <- source
  ...
  if (!is.list(parameters)) {
    log_message(
      "Spatial result {.arg parameters} must be a list",
      message_type = "error"
    )
  }
  ...
  if (length(provenance$backend_versions) == 0L) {
    provenance$backend_versions <- spatial_result_backend_versions(
...
spatial_result_validate <- function(bundle) {
  ...
  if (!identical(as.integer(bundle$schema_version), 1L)) {
    log_message(
      "Unsupported spatial result schema version",
      message_type = "error"
    )
  }
  if (
    !is.list(bundle$source) ||
      !is.list(bundle$provenance) ||
      !is.list(bundle$parameters)
  ) {
    log_message(
      "Spatial result source, provenance, and parameters must be lists",
      message_type = "error"
    )
  }
  invisible(TRUE)
...
spatial_result_registry_row <- function(tool_name = NULL, bundle = NULL) {
  ...
  producer <- if (is.list(bundle)) {
    bundle$provenance$producer %||% NA_character_
  } else {
    NA_character_
  }
  family <- if (is.list(bundle)) {
    bundle$method %||% NA_character_
  } else {
    NA_character_
  }
  match_index <- integer()
  if (!is.null(tool_name)) {
    match_index <- which(registry$tool_key == tool_name)
  }
...
spatial_result_normalize <- function(
  bundle,
  tool_name = NULL,
  registry_row = NULL
) {
  if (
    is.list(bundle) &&
      identical(as.integer(bundle$schema_version %||% NA_integer_), 1L)
  ) {
    spatial_result_validate(bundle)
    return(bundle)
  }
  registry_row <- registry_row %||%
    spatial_result_registry_row(tool_name, bundle)
  if (is.null(registry_row)) {
    log_message(
      "Stored tool {.val {tool_name}} is not a registered spatial result",
      message_type = "error"
    )
  }
  ...
  source <- utils::modifyList(
    list(
      ...
      coordinate_space = parameters$coordinate_space %||%
        registry_row$coordinate_space_current[[1L]],
      transform = NULL,
      assay = parameters$assay %||% NA_character_,
      layer = parameters$layer %||% NA_character_
    ),
...
spatial_result_index <- function(object) {
  ...
  if (length(rows) == 0L) {
    return(data.frame(
      tool_name = character(),
      registry_method = character(),
      family = character(),
      result_type = character(),
      backend_id = character(),
      plot_function = character(),
      coordinate_space_current = character(),
      stringsAsFactors = FALSE
    ))
  }
  do.call(rbind, rows)
...
spatial_result_state <- function(bundle, object_cells = NULL) {
  ...
  if (!is.list(bundle)) {
    return(list(
      state = if (length(bundle) > 0L) "ready" else "empty",
      n_items = length(bundle),
      reason = NA_character_
    ))
  }
  if (!is.null(bundle$graphs) && length(bundle$graphs) > 0L) {
    node_ids <- unique(unlist(
      lapply(bundle$graphs, function(graph) {
        nodes <- graph$nodes
        if (is.data.frame(nodes) && "cell_id" %in% colnames(nodes)) {
          as.character(nodes$cell_id)
        } else {
          character()
        }
      }),
    ...
    stale <- length(node_ids) > 0L &&
      !is.null(object_cells) &&
      any(!node_ids %in% object_cells)
    return(list(
      state = if (stale) "stale" else "ready",
      n_items = length(bundle$graphs),
      reason = if (stale) {
        "stored graph nodes are absent from object"
      } else {
        NA_character_
      }
    ))
  ...
  if (
    length(payload_ids) > 0L &&
      !is.null(object_cells) &&
      any(!payload_ids %in% object_cells)
  ) {
    return(list(
      state = "stale",
  ...
  payload_sizes <- vapply(
    bundle[payload_names],
    spatial_result_payload_size,
    integer(1)
  )
  ...
  if (n_items == 0L) {
    return(list(
      state = "empty",
      n_items = 0L,
      reason = "no logical result payload"
    ))
  }
  partial <- is.null(bundle$method) ||
    is.null(bundle$source) ||
    is.null(bundle$parameters)
  list(
...
GetSpatialResult <- function(
  ...
  if (is.null(method) && is.null(tool_name)) {
    log_message(
      "Provide {.arg method} or {.arg tool_name}",
      message_type = "error"
    )
  }
  ...
  if (!is.null(tool_name)) {
    keep <- keep & index$tool_name %in% tool_name
  }
  matches <- index[keep, , drop = FALSE]
  if (nrow(matches) == 0L) {
    log_message(
      "No stored spatial result matches the requested selector",
      message_type = "error"
    )
  }
  if (nrow(matches) > 1L) {
  ...
  if (isTRUE(validate)) {
    spatial_result_validate(normalized)
  }
  normalized
}
...
SpatialResultInfo <- function(
  ...
  if (
    !is.logical(include_empty) ||
      length(include_empty) != 1L ||
      is.na(include_empty)
  ) {
    log_message(
      "{.arg include_empty} must be TRUE or FALSE",
      message_type = "error"
    )
  }
  ...
  if (!is.null(tool_name)) {
    index <- index[index$tool_name %in% tool_name, , drop = FALSE]
  }
  ...
  if (identical(detail, "graphs")) {
    rows <- lapply(seq_len(nrow(index)), function(i) {
      ...
      if (is.null(graph_names)) {
        graph_names <- as.character(seq_along(bundle$graphs))
      }
      do.call(
        rbind,
        lapply(seq_along(bundle$graphs), function(j) {
  ...
  rows <- lapply(seq_len(nrow(index)), function(i) {
    ...
    data.frame(
      ...
      tool_name = key,
      schema_version = if (is.list(stored)) {
        as.integer(stored$schema_version %||% NA_integer_)
      } else {
        NA_integer_
      },
      result_state = state$state,
      n_items = state$n_items,
      coordinate_space = as.character(
        source$coordinate_space %||% index$coordinate_space_current[[i]]
      )[[1L]],
      image = paste(source$image %||% character(), collapse = ","),
  ...
  if (length(rows) == 0L) {
    return(data.frame(
      ...
      result_state = character(),
      n_items = integer(),
      coordinate_space = character(),
      image = character(),
      parameters_available = logical(),
