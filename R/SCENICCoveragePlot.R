...
scenic_get_region_gene_links <- function(srt, tool_name) {
  ...
  gene_col <- scenic_first(intersect(c("gene", "target", "Gene"), colnames(links)))
  if (is.null(gene_col)) {
    log_message(
      "Region-gene table must contain a {.val gene} or {.val target} column",
      message_type = "error"
    )
  }
  score_col <- scenic_first(intersect(c("score", "importance", "weight", "rho"), colnames(links)))
  tf_col <- scenic_first(intersect(c("TF", "tf", "regulator"), colnames(links)))
  out <- data.frame(
    ...
    score = if (is.null(score_col)) {
      ...
      as.numeric(links[[score_col]])
    },
    TF = if (is.null(tf_col)) {
      rep(NA_character_, nrow(links))
    } else {
      as.character(links[[tf_col]])
    },
...
scenic_chromatin_assay <- function(srt, atac_assay = NULL) {
  ...
  for (nm in c("peaks", "ATAC", "atac", "Peaks")) {
    if (nm %in% assays && inherits(srt[[nm]], "ChromatinAssay")) {
      return(nm)
    }
  }
  chromatin <- assays[vapply(assays, function(nm) inherits(srt[[nm]], "ChromatinAssay"), logical(1))]
  if (length(chromatin) > 0L) {
    return(chromatin[[1L]])
  }
  NULL
}
...
scenic_annotation_gene_coords <- function(srt, atac_assay, genes) {
  ...
  name_col <- scenic_first(intersect(
    c("gene_name", "gene_id", "gene", "tx_id"),
    colnames(ann_df)
  ))
  if (is.null(name_col)) {
    return(NULL)
