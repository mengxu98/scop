RunBulk_deconv_music <- function(
  ...
  if (identical(backend, "native")) {
    return(.bulk_method_failed(
      reason = paste(
        "The native MuSiC backend is not implemented yet.",
  ...
  log_message(
    "{.val deconv_MuSiC} currently uses SCOP internal profile fitting.",
    verbose = verbose
  )
