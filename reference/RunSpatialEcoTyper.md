...
- x.by, y.by:
  ...
  Metadata columns containing single-cell spatial coordinates. Used only
  when no image is present; image-backed discovery uses raw image
  coordinates through
...
- image:
  ...
  Optional image name for single discovery, or named sample-to-image map
