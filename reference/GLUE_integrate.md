...
Integrate paired RNA and ATAC data using `scglue`. The current
implementation supports one `Seurat` object containing one RNA assay and
one `ChromatinAssay`, and folds the modality embeddings back to the
...
- gene_annotation_by:
  ...
  How RNA feature names are matched to gene annotation. One of `"auto"`,
  `"gene_name"`, or `"gene_id"`. Default is `"auto"`.

- GLUE_params:

  A list of parameters passed to `scglue.models.fit_SCGLUE()`. Reserved
  keys are `"adatas"` and `"graph"`.
