...
- max_cells:
  ...
  predicts fluxes for all cells in batches. This drastically reduces
  peak memory for large datasets. Set \`NULL\` (default) to train on all
  cells, matching the original upstream behaviour. A value such as
...
The scFEA GNN architecture, loss function, training loop, and M168
human/mouse model files are downloaded from the \`mengxu98/datasets\`
