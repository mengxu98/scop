...
Runs an enhanced scVelo workflow on a Seurat object with improved error
handling, version compatibility, and modular design.
...
- log_transform:
  ...
  Whether to apply log transformation to AnnData `X` in the Python
  backend. The C++ velocity calculation uses spliced/unspliced layers,
  matching scVelo, so this does not alter its velocity matrices.
...
- compute_velocity_graph:
  ...
  terminal-state or pseudotime calculations. If `NULL`, compute and
  store the graph only when terminal states or pseudotime are requested.
...
- max_dense_gib:
  ...
  Maximum estimated GiB allowed for the dense expression working
