...
Performs single-cell reference mapping using the K-nearest neighbor
(KNN) method. It takes two single-cell datasets as input: srt_query and
srt_ref. The function maps cells from the srt_query dataset to the
srt_ref dataset based on their similarity or distance.
...
- query_reduction:
  ...
  Name of a dimensionality reduction in the query object to use for
  calculating the distance metric.

- ref_reduction:

  Name of a dimensionality reduction in the reference object to use for
  calculating the distance metric.

- query_dims:

  Dimension indices from the query reduction to be used for calculating
  the distance metric.
...
#> ℹ [2026-09-20 22:38:09] Skip `log1p()` because `layer = data` is not "counts"
...
#> ℹ [2026-09-20 22:38:17] Data type is log-normalized
#> ℹ [2026-09-20 22:38:17] Data type is log-normalized
ProjectionPlot(
  srt_query = srt_query,
  srt_ref = srt_ref,
