...
#> ℹ [2026-01-30 16:50:36] Start standard scop workflow...
#> ℹ [2026-01-30 16:50:37] Checking a list of <Seurat>...
#> ! [2026-01-30 16:50:37] Data 1/1 of the `srt_list` is "unknown"
#> ℹ [2026-01-30 16:50:37] Perform `NormalizeData()` with `normalization.method = 'LogNormalize'` on the data 1/1 of the `srt_list`...
#> ℹ [2026-01-30 16:50:39] Perform `Seurat::FindVariableFeatures()` on the data 1/1 of the `srt_list`...
#> ℹ [2026-01-30 16:50:40] Use the separate HVF from srt_list
#> ℹ [2026-01-30 16:50:40] Number of available HVF: 2000
#> ℹ [2026-01-30 16:50:40] Finished check
#> ℹ [2026-01-30 16:50:40] Perform `Seurat::ScaleData()`
#> ℹ [2026-01-30 16:50:41] Perform pca linear dimension reduction
#> ℹ [2026-01-30 16:50:41] Perform `Seurat::FindClusters()` with `cluster_algorithm = 'louvain'` and `cluster_resolution = 0.6`
#> ℹ [2026-01-30 16:50:42] Reorder clusters...
#> ℹ [2026-01-30 16:50:42] Perform umap nonlinear dimension reduction
#> ℹ [2026-01-30 16:50:42] Non-linear dimensionality reduction (umap) using (Standardpca) dims (1-50) as input
#> ℹ [2026-01-30 16:50:45] Non-linear dimensionality reduction (umap) using (Standardpca) dims (1-50) as input
#> ✔ [2026-01-30 16:50:49] Run scop standard workflow completed
FeatureStatPlot(
  pancreas_sub,
...
#> ℹ [2026-01-30 16:51:12] Detected more than 2 groups. Use "kruskal.test" for comparison
#> Warning: Groups with fewer than two datapoints have been dropped.
#> ℹ Set `drop = FALSE` to consider such groups for position adjustment purposes.
#> Warning: Groups with fewer than two datapoints have been dropped.
#> ℹ Set `drop = FALSE` to consider such groups for position adjustment purposes.
...
#> ℹ [2026-01-30 16:51:30] Setting `group.by` to "Features" as `plot.by` is set to "feature"
#> Warning: No shared levels found between `names(values)` of the manual scale and the
#> data's colour values.
#> Warning: No shared levels found between `names(values)` of the manual scale and the
#> data's colour values.
...
#> ℹ [2026-01-30 16:51:31] Setting `group.by` to "Features" as `plot.by` is set to "feature"
#> Warning: No shared levels found between `names(values)` of the manual scale and the
#> data's colour values.
#> Warning: No shared levels found between `names(values)` of the manual scale and the
#> data's colour values.
...
#> ℹ [2026-01-30 16:51:34] Setting `group.by` to "Features" as `plot.by` is set to "feature"
...
#> ℹ [2026-01-30 16:51:37] Setting `group.by` to "Features" as `plot.by` is set to "feature"
