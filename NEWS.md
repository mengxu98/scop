...
* **fix**: Giotto bridges reuse an already-installed GiottoClass instead of reinstalling `drieslab/Giotto` when the GitHub remote does not match (for example the optional CI pin `giotto-suite/Giotto`). Documented examples probe `GiottoClass` with `install = FALSE` and convert only when it is already present. The live round-trip test uses a tiny in-process fixture instead of reloading the source tree under a 120s `callr` timeout.
