...
* **fix**: Remove the unverified `RunSpatialDWLS()` approximation and its
  result plotting path rather than presenting clipped least-squares output as
  the published SpatialDWLS method. Spatial deconvolution examples now use
  retained validated producers. Remove redundant SpatialEcoTyper plotting
  wrappers and keep spatial-gradient plotting focused on stored summary,
  distance-curve, and model-fit results.
...
* **feat**:
  ...
  * Real-data consistency tests verify the wrappers against the original pipelines (CellChat, SpatialCellChat, LIANA, BayesSpace, MERINGUE, SPOTlight, SpaNorm, RCTD, BANKSY, CARD, SpotSweeper, PRECAST).
...
* **fixed**:
  ...
  * Coordinate-contract validation covers unified CCC plots for SpatialCellChat, COMMOT, and SpaTalk. Explicit `coordinate_space = "legacy_display"` analysis preserves its historical lowres-to-hires fallback; current display plotting remains strict about the selected scale.
