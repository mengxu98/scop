...
* **feat**:
  ...
  * `RunIntegrationBenchmark()` compares integration methods with scIB-style iLISI/cLISI, ASW, graph connectivity, and ARI/NMI scores. `IntegrationBenchmarkPlot()` draws overview, heatmap, scatter, UMAP, and LISI views. Printing uses `thisplot::print_colored_table()`. Spatial clustering benchmarks remain in `RunBenchmark()` / `BenchmarkPlot()`.
