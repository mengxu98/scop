...
* **feat**:
  ...
  * `SCENICPlot()` gains official-style `"heatmap_dotplot"` (TF expression vs RSS) and `"eregulon_dim"` (TF expression, gene AUC, and region AUC when stored). RSS/target/size plots use `palette_colors()`; `activity_dim` can sit beside TF expression; networks fall back to SCENIC+ `tf_gene` / `triplets`. New `SCENICPlusPlot()` wraps the same views with SCENIC+ assay defaults.
* **fixed**:
