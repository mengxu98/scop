...
* **fix**:
  ...
  * `RunscPagwas()` now prefers the upstream `scPagwas_main2` runner, accepts validated GWAS data-frame or file inputs, uses `group.by` to set cell identities, forwards single-cell, cell-type, assay, and return controls explicitly, and applies local Seurat 5 compatibility without mutating optional-package namespaces. `PlotscPagwas()` provides score and significance embedding plots through SCOP's dimensional plotting functions and palette system.
