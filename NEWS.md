...
* **feat**:
  ...
  * Updated the C++ PLAGE standardization path to match GSVA 2.6 complete-row scaling while keeping the version-specific parity test explicit for older GSVA releases.
...
* **fix**:
  ...
  * `GetAssayData5.Assay5()` no longer unconditionally calls `JoinLayers()`, preserving BPCells on-disk/lazy layers when the requested layer can be read directly. Methods that must materialize BPCells matrices now use explicit warning helpers before converting to dense matrices or `dgCMatrix`, highlighting memory, tempdir, and 2^31-entry risks.
