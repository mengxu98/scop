...
* **feat**:
  ...
  * `CellStatPlot()` now exposes `venn_engine` and `venn_args` for forwarding to `thisplot::StatPlot()`, enabling the optional `venny` Venn backend and its styling controls.
