...
* **feat**:
  * Added `RunCHOIR()` as a runtime-optional wrapper for significance-based hierarchical clustering of Seurat objects, with pinned backend provenance, strict result validation, metadata writeback, and optional `srt@tools` summaries.
  ...
  * Renamed `db_scDblFinder()`, `db_scds()`, `db_Scrublet()`, and `db_DoubletDetection()` to `RunscDblFinder()`, `Runscds()`, `RunScrublet()`, and `RunDoubletDetection()`. The old entry points now emit deprecation warnings and will be removed in version 1.0.0.
  * `RunIntegration()` now accepts multiple methods through `integration_methods` and appends their outputs sequentially. The former `integration_method` argument remains as a deprecated compatibility alias until version 1.0.0.
  ...
  * `RunSpatialNeighborhood()` now defaults to observed-neighborhood summaries computed in the package; `method = "spicyR"` requires `split.by` and records spicyR provenance only after the backend runs. The standard spatial workflow now records per-stage requested/completed/skipped/failed state and reports partial completion when a requested stage is skipped.
  ...
  * Added `RunSpatialCellChat()` as a runtime-optional SpatialCellChat v3 producer for cell-, spot-, and composition-level communication. It enforces raw-to-micron coordinate contracts, isolates samples before distance calculations, stores truthful schema-v1 provenance, keeps upstream objects optional, and publishes group-level results under the distinct `SpatialCellChat` CCC method. `GetCCCObject()` and `SpatialCellChatPlot()` provide upstream-object access and stored-result spatial views without rerunning the backend.
  ...
  * Added `MistyRPlot()` and `StatialKontextualPlot()` to close the visualization loop for stored high-level spatial context results without wrapping or rerunning their upstream frameworks.
  * Added `RunCell2location()` for official Python cell2location reference-signature learning and spatial abundance mapping through a persisted subprocess workflow. The wrapper records q05 absolute abundance and normalized proportions, supports manifest-validated resume and signature reuse, integrates with `RunStandardWorkflow()`, and adds `Cell2locationPlot()` for abundance, proportion, dominant-type, and pie maps. `PrepareEnv(modules = "cell2location")` installs cell2location 0.1.5 with scvi-tools 1.3.3 in the shared SCOP environment.
  ...
  * Added `ClusterTreePlot()` for visualization of Seurat multi-resolution clustering trees, including automatic `*_snn_res.*` metadata detection, prefix/resolution filtering, edge contribution statistics, and marker-expression overlays.
  ...
  * Added `RunSciBet()` for a C++ SciBet-compatible single-cell classifier using reference/query Seurat objects or expression matrices, with prediction metadata written back to Seurat.
  ...
  * `RunPAGA()` supports a C++ backend for the standard PAGA connectivity graph; `backend = "python"` remains the default and is required for RNA-velocity transitions, PAGA-initialized embeddings, plotting side effects, and the reference workflow.
  ...
  * Added `RunCytoSPACE()`, an R/C++ implementation of the default CytoSPACE spot-level assignment workflow. C++ is used for spot-capacity assignment and can also be selected for cell-fraction estimation; detailed results are stored in `srt@tools[["CytoSPACE"]]` with summary metadata columns using the requested prefix.
  * Added `RunRCTD()` and `RunSPOTlight()` spatial deconvolution wrappers for estimating spot-level cell-type proportions from spatial Seurat objects and single-cell references, with standardized metadata columns compatible with `SpatialSpotPlot()` and `RunStandardWorkflow(spatial_deconv_method = ...)`.
  * Added `RunSpatialEcoTyper()` as an optional SpatialEcoTyper wrapper for single-sample discovery, multi-sample conserved SE discovery, pretrained SE recovery, and SE abundance deconvolution. Results are written back to Seurat metadata and `srt@tools`, and `SpatialEcoTyperSpatialPlot()`/`SpatialEcoTyperCompositionPlot()` provide compatible visualization helpers.
  * Added `RunSpatialGradientFeatures()` and `SpatialGradientPlot()` for SPATA2-compatible spatial gradient feature screening, including a C++ screening path, trajectory or annotation-based screening modes, model-fit summaries, and spatial/line/profile visualizations.
...
* **fix**:
  * Fixed source installation with recent macOS/Xcode toolchains by preventing R's legacy `length()` macro from rewriting libc++ declarations in the bundled CIBERSORT backend.
  * `RunDEtest()` now routes supported Wilcoxon `all`, `paired`, `conserved`, and `disturbed` analyses through scop's sparse marker backends regardless of whether `presto` is installed. The all-group path reuses the all-in-one kernel, symmetric paired comparisons are computed once, conserved strata no longer call `Seurat::FindMarkers()` directly, and zero-variance genes receive finite p-values.
  * Hardened `RunCHOIR()` and `RunCell2fate()` against unsafe output replacement, stale or concurrent runs, malformed backend results, and incomplete resumable artifacts. Cell2fate now publishes trained outputs from a staging directory, verifies persisted artifact hashes, and safely accepts its own previously returned Seurat object for resume.
  * `CellDimPlot()` and `FeatureDimPlot()` now keep automatically sized points readable for large datasets: non-raster points have a `0.3` minimum size and automatically sized raster layers use at least a two-pixel diameter. Explicit `pt.size` values remain unchanged.
  * `RunscPagwas()` now prefers the upstream `scPagwas_main2` runner, accepts validated GWAS data-frame or file inputs, uses `group.by` to set cell identities, forwards single-cell, cell-type, assay, and return controls explicitly, and applies local Seurat 5 compatibility without mutating optional-package namespaces. `PlotscPagwas()` provides score and significance embedding plots through the package dimensional plotting functions and palette system.
  * Hardened live spatial-backend adapters without making optional repositories hard dependencies. `RunCARD()` now preserves the full spot identity while marking CARDspa-ineligible informative-zero spots explicitly, `RunSpatialEcoTyper()` supplies locally scoped namespace compatibility for upstream dplyr omissions, `RunSpatialCellChat()` retains valid pathway-level communication when ligand-receptor extraction is unavailable and avoids global progress-handler mutation, and `RunSpatialQM()` now checks its registered GitHub repository, filters arguments against the installed metric API, maps finite metadata coordinates into its temporary input, and defaults to the object-first metrics supported by the current upstream release.
