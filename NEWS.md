...
* **fixed**:
  * Spatial coordinate contract v2 now reads full-resolution VisiumV1/VisiumV2 coordinates before applying exactly one selected `lowres` or `hires` display transform. `SpatialCoordinates()`, HE-overlay point/pie/network plots, and their public wrappers gain `image.scale`; users no longer need to modify Seurat `@scale.factors` slots. Coordinate-dependent stored results created under an older or unknown contract must be rerun.
