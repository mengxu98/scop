...
* **spatial inputs**: `ReadSpatialData()` delegates Visium, Visium HD and Xenium loading to Seurat and records analysis units, assay/image resolution and coordinate units. `SpatialDataInfo()` validates the selected context; `SpatialSegmentationQC()` reports polygon area/count QC without silently filtering cells or claiming missing boundaries passed.
* **large spatial data**: `RunSpatialSketch()` scales only the sampled assay and projects Seurat labels to the full selected assay. `ScaleData()` now supports Assay5 cell subsets and updates layer membership on rescaling. SPARK-X retains sparse input, and dense spatial-feature conversions have an explicit size guard.
...
* **sample inference**: `SpatialSampleSummary()`, `SpatialSampleComparison()` and `SpatialSamplePlot()` summarize samples, average repeated sections within subjects, and support independent or paired two-condition comparisons. Insufficient subject replication is reported as not tested. Neighborhood summaries retain undefined empty neighborhoods as missing values.
