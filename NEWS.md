...
* **feat**:
  ...
  * `SCENICPlot()` gains official-style `"heatmap_dotplot"` (TF expression vs RSS), `"eregulon_dim"` (TF expression, gene AUC, and region AUC when stored), and `"coverage"` (ATAC accessibility plus region–gene link tracks). Network plots follow [EnrichmentPlot()] styling (TF-colored edges, target `geom_label()` pills, double-ring TF hubs). `"network"` uses per-TF star hubs, `"network_graph"` uses multi-hub layout, `"egrn"` draws TF–region–gene GRNs, and `"overlap"` shows eRegulon Jaccard overlap. RSS/target/size plots use `palette_colors()`; `activity_dim` can sit beside TF expression; networks fall back to SCENIC+ `tf_gene` / `triplets`. New `SCENICPlusPlot()` wraps the same views with SCENIC+ assay defaults.
* **fixed**:
