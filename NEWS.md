...
* **feat**:
  ...
  * `RunSCENIC()`: Added a `genome` parameter for automatic cisTarget reference selection while keeping `species` aligned with the package-wide Latin-name style. Human references now support the default `"hg38"` v10 databases and `"hg19"` v9 databases, while the human TF list is cached with the genome-neutral name `allTFs_hgnc.txt`.
