...
* **feat**:
  ...
  * `DorotheaPlot()` now follows the shared scop plot style: horizontal `"bar"`/`"lollipop"` comparison plots, `"volcano"`, and `"heatmap"` via `GroupHeatmap()`. `RunDorothea()` examples show `FeatureDimPlot()` on the TF-activity assay.
* **fixed**:
