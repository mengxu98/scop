...
* **feat**:
  ...
  * `SCENICPlot()` gains official-style `"heatmap_dotplot"` (TF expression vs RSS), `"eregulon_dim"` (TF expression, gene AUC, and region AUC when stored), and `"coverage"` (ATAC accessibility plus region–gene link tracks). Network plots follow published SCENIC+/iRegulon figures (Nature Methods 2023 Fig. 2e; PLOS Comput. Biol. iRegulon Fig. 3): concentric TF hubs, unlabeled region diamonds, light-gray gene nodes, TF-colored edges, and Kamada–Kawai eGRNs. Multi-TF networks add a legend of TF colors with the RSS top cell type; single-TF hubs put the cell type in the title. Discrete network colors use the scop `"Chinese"` palette. `SCENICPlot()` / `SCENICPlusPlot()` examples cover multi-TF `"egrn"` and `"network_graph"`. `"network"` uses per-TF star hubs, `"network_graph"` uses a global force layout, `"egrn"` draws TF–region–gene GRNs, and `"overlap"` shows eRegulon Jaccard overlap. RSS/target/size plots use `palette_colors()`; `activity_dim` can sit beside TF expression; networks fall back to SCENIC+ `tf_gene` / `triplets`. New `SCENICPlusPlot()` wraps the same views with SCENIC+ assay defaults.
* **fixed**:
...
* **feat**:
  ...
  * Added `RunBenchmark()` and `BenchmarkPlot()` for failure-tolerant comparison of stable spatial-domain clustering methods (ARI/NMI/purity, runtime, peak memory); added `ClusterTreePlot()` for multi-resolution clustering trees.
