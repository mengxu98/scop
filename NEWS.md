...
* **feat**:
  ...
  * `SCENICPlot()` gains official-style `"heatmap_dotplot"` (TF expression vs RSS), `"eregulon_dim"` (TF expression, gene AUC, and region AUC when stored), and `"coverage"` (ATAC accessibility plus region–gene link tracks). Network plots follow published SCENIC/SCENIC+ figures: `"network"` uses per-TF star hubs (inner region ring when triplets are stored), `"network_graph"` uses a multi-hub layout, `"egrn"` draws the TF–region–gene GRN, and `"overlap"` shows eRegulon target Jaccard overlap. RSS/target/size plots use `palette_colors()`; `activity_dim` can sit beside TF expression; networks fall back to SCENIC+ `tf_gene` / `triplets`. New `SCENICPlusPlot()` wraps the same views with SCENIC+ assay defaults.
* **fixed**:
